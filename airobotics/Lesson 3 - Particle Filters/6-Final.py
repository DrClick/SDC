...
landmarks = [[0.0, 100.0], [0.0, 0.0], [100.0, 0.0],
             [100.0, 100.0]]  # position of 4 landmarks in (y, x) format.
...
class robot:
    ...
    def measurement_prob(self, measurements):
        ...
        for i in range(len(measurements)):
            ...
            # update Gaussian
            error *= (
            exp(- (error_bearing ** 2) / (self.bearing_noise ** 2) / 2.0) /
    ...
    #           self.steering_noise
    #           self.distance_noise
    def turning_angle(self, distance, steering_angle):
        """calcs the turning angle beta"""
        return distance/self.length * tan(steering_angle)

    def turning_radius(self, distance, steering_angle):
        """calcs the turning radius R"""
        beta = self.turning_angle(distance, steering_angle)
        return distance/beta

    def point_of_rotation(self, x_pos, y_pos, distance, steering_angle, heading_angle):
        R = self.turning_radius(distance, steering_angle)
        cx = x_pos - sin(heading_angle) * R
        cy = y_pos + cos(heading_angle) * R

        return cx, cy

    def update_turn(self, steer_angle, distance, x_pos, y_pos, theta):
        beta = self.turning_angle(distance, steer_angle)
        R = self.turning_radius(distance, steer_angle)
        cx, cy = self.point_of_rotation(x_pos, y_pos, distance, steer_angle, theta)

        _x = cx + sin(theta + beta) * R
        _y = cy - cos(theta + beta) * R
        _theta = (theta + beta) % (2 * pi)

        return _x, _y, _theta

    def update_straight(self, steer_angle, distance, x_pos, y_pos, theta):
        beta = self.turning_angle(distance, steer_angle)

        _x = x_pos + distance * cos(theta)
        _y = y_pos + distance * sin(theta)
        _theta = (theta + beta) % (2 * pi)
    ...
    def move(self, motion):  # Do not change the name of this function
        ...
        theta = random.gauss(theta, self.bearing_noise)

        if steer_angle < .001:
            _x, _y, _theta = self.update_straight(steer_angle, distance, x, y, theta)
        else:
            _x, _y, _theta = self.update_turn(steer_angle, distance, x, y, theta)
    ...
    # according to
    #           self.bearing_noise
    def sense(self, noisy=False):
            delta_landmarks = [(y - self.y, x - self.x) for (y, x) in landmarks]
            result = [(atan2(y, x) - self.orientation) for (y, x) in delta_landmarks]
            # correct so all angles are positive
            result = [x % (2 * pi) for x in result]

            if noisy:
                result = [random.gauss(x, self.bearing_noise) for x in result]
...
def print_measurements(Z):
    ...
    print '                [%.8s, %.8s, %.8s, %.8s]]' % \
          (str(Z[T - 1][0]), str(Z[T - 1][1]), str(Z[T - 1][2]),
           str(Z[T - 1][3]))
...
def particle_filter(motions, measurements,
